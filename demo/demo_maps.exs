...
IO.puts("   Person.fetch!(:name) => #{inspect Person.fetch!(:name)}")
IO.puts("   Person.fetch!(:age) => #{inspect Person.fetch!(:age)}")
IO.puts("   Person.fetch!(:city) => #{inspect Person.fetch!(:city)}")
IO.puts("   ")
...
IO.puts("   AppSettings.fetch!(:config) => #{inspect AppSettings.fetch!(:config)}")
IO.puts("   AppSettings.fetch!(:features) => #{inspect AppSettings.fetch!(:features)}")
IO.puts("")

# Example 3: Custom function name
IO.puts("3. Standard API functions:")
user_data = %{username: "bob", role: :admin, active: true}

{:ok, _module} = Maps.convert(user_data, :User)

IO.puts("   User.fetch(:username) => #{inspect User.fetch(:username)}")
IO.puts("   User.fetch(:role) => #{inspect User.fetch(:role)}")
IO.puts("   User.fetch(:active) => #{inspect User.fetch(:active)}")
IO.puts("   ")
IO.puts("   Bang versions for direct access:")
IO.puts("   User.fetch!(:username) => #{inspect User.fetch!(:username)}")
IO.puts("   User.fetch!(:role) => #{inspect User.fetch!(:role)}")
...
IO.puts("   #{module1}.fetch!(:env) => #{inspect module1.fetch!(:env)}")
IO.puts("   Second config module: #{inspect module2}")
IO.puts("   #{module2}.get(:env) => #{inspect module2.get(:env)}")
IO.puts("   #{module2}.fetch!(:env) => #{inspect module2.fetch!(:env)}")
IO.puts("")
...
IO.puts("   MixedKeys.fetch!(:atom_key) => #{inspect MixedKeys.fetch!(:atom_key)}")
IO.puts("   MixedKeys.fetch!(\"string_key\") => #{inspect MixedKeys.fetch!("string_key")}")
IO.puts("   MixedKeys.fetch!(1) => #{inspect MixedKeys.fetch!(1)}")
IO.puts("")
...
IO.puts("   LargeData.fetch!(:item_10) => #{inspect LargeData.fetch!(:item_10)}")
IO.puts("   LargeData.fetch!(:item_50) => #{inspect LargeData.fetch!(:item_50)}")
...
IO.puts("  - Person.get(:name) or Person.fetch!(:name)")
IO.puts("  - AppSettings.get(:config) or AppSettings.fetch!(:config)")
IO.puts("  - Safe versions return {:ok, value} or nil")
IO.puts("  - Bang versions return value directly or raise KeyError")

# Examples of get/2 (returns value directly, not {:ok, value})
IO.puts("\n9. Demonstrating get/2 with default values:")
IO.puts("   Person.get(:name) => #{inspect Person.get(:name)}")
IO.puts("   Person.get(:nonexistent) => #{inspect Person.get(:nonexistent)}")
IO.puts("   Person.get(:nonexistent, :default) => #{inspect Person.get(:nonexistent, :default)}")
